 ...
 * @author josue
 */
public class DemoButtons extends javax.swing.JFrame {
  ...
  static class SignatureDialog extends JDialog implements ITabletHandler
  {
    ...
    public SignatureDialog(Frame frame, UsbDevice usbDevice, TlsDevice tlsDevice, boolean useSigMode) throws STUException
    {
      super(frame, true);
      ...
      try
      {
        ...
        {
          throw new RuntimeException("Error, fallo la conexión con el usb de la tablet !! " + e);
        }
  ...
  private void onGetSignature()
  {
    try
    {
      ...
      if (usbDevice != null || tlsDevice != null)
      {
        ...
        SignatureDialog signatureDialog = new SignatureDialog(this, usbDevice,tlsDevice, sigMode);
        ...
        if (penData != null && penData.length > 0)
        {
		  ...
		  try
		  {
			  ImageIO.write(this.signatureImage, "png", new File("C:\\hmsoft\\firma.png"));
		  }
		  catch (Exception e)
		  {
		  }
      ...
      {
        throw new RuntimeException("No hay tablets sincronizadas!!");
      }
    }
    catch (STUException e)
    {
  ...
  public DemoButtons()
  {
    this.setTitle("INTEGRACION CON EL STU SDK");
    this.setLayout(new BorderLayout());
    ...
    JButton btn = new JButton("Modo Signature");
    ...
    chkUseSigMode = new JCheckBox("Usar Modo Signature");
